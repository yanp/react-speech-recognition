...
var transcriptReducer = function transcriptReducer(state, action) {
  switch (action.type) {
    ...
    case _constants.APPEND_TRANSCRIPT:
      newFinalTranscriptAlternatives = action.payload.finalTranscriptAlternatives.length > 0 ? [].concat(_toConsumableArray(state.finalTranscriptAlternatives), [action.payload.finalTranscriptAlternatives]) : state.finalTranscriptAlternatives;
      return {
