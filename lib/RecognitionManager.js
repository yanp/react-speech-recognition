...
var RecognitionManager = /*#__PURE__*/function () {
  ...
  _createClass(RecognitionManager, [{
    ...
    value: function emitTranscriptChange(interimTranscript, finalTranscript, finalTranscriptAlternatives) {
      var _this3 = this;
    ...
    value: function updateTranscript(_ref) {
      ...
      for (var i = currentIndex; i < results.length; ++i) {
        if (results[i].isFinal && (!(0, _isAndroid["default"])() || results[i][0].confidence > 0)) {
          ...
          for (var j = currentIndex; j < results[i].length; ++j) {
    ...
    key: "updateFinalTranscriptAlternatives",
    value: function updateFinalTranscriptAlternatives(newFinalTranscriptAlternatives) {
